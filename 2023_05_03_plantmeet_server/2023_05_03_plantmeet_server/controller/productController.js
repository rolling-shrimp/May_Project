...
module.exports = {
  ...
  productTypeDetail: function (req, res) {
    const productType = req.params.product_type;
    productData.getProductByTypeDetail(req.con, productType, req.params.id, function (err, result) {
      if (err) {
        res.send({
          err: err,
        });
  ...
  imageUpdate: function (req, res) {
    productData.updateImage(
      req.con, req.body, req.params.id, function (err, result) {
        if (err) {
          res.send({
            err: err,
          });
